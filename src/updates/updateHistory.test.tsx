...
describe('UPDATE_HISTORY', () => {
  it('records the current learner-facing improvement first', () => {
    expect(UPDATE_HISTORY[0]).toEqual({
      date: '2026-08-31',
      category: '개선',
      summaries: [
        '정상적인 비교 시작 화면과 완료 결과가 헷갈리지 않도록 안내를 고쳤습니다.',
        '작은 화면의 업데이트 내역 창에서도 제목과 닫기 버튼이 겹치지 않게 다듬었습니다.',
      ],
    });
  });
  ...
  it('keeps the exact dated entries newest first and immutable', () => {
    expect(UPDATE_HISTORY).toEqual([
      { date: '2026-08-31', category: '개선', summaries: ['정상적인 비교 시작 화면과 완료 결과가 헷갈리지 않도록 안내를 고쳤습니다.', '작은 화면의 업데이트 내역 창에서도 제목과 닫기 버튼이 겹치지 않게 다듬었습니다.'] },
...
describe('UpdateHistoryButton', () => {
  ...
  it('keeps the dialog title and close action in one named header', async () => {
    const user = userEvent.setup();
    render(<UpdateHistoryButton />);
    await user.click(screen.getByRole('button', { name: '업데이트 내역' }));
    const header = document.querySelector('.update-history-dialog-header');
    expect(header).toBeInTheDocument();
    expect(header?.querySelector('h2')).toHaveTextContent('업데이트 내역');
    expect(header?.querySelector('button')).toHaveAccessibleName('업데이트 내역 닫기');
  });
