...
const isPlacementComplete = (state: SessionState): boolean => {
  ...
  for (const placement of state.placements) {
    ...
    if (placement.slotId.trim().length === 0 || candidate === undefined || slotIds.has(placement.slotId) || candidateIds.has(placement.candidateId) || !mission.facilityKinds.includes(placement.facilityKind)) return false;
...
const isFreshAnalysis = (state: SessionState, analysis: PlacementAnalysis | null): analysis is PlacementAnalysis => {
  if (analysis === null || state.cityId === null || state.missionId === null) return false;
  const mission = MISSIONS[state.missionId];
  const context = analysis.missionContext;
  return analysis.cityId === state.cityId
    && analysis.missionId === state.missionId
    && samePlacements(analysis.placements, state.placements)
    && context !== undefined
    && context.budgetTokens === mission.budgetTokens
    && context.serviceThreshold === mission.serviceThreshold
    && context.facilityKinds.length === mission.facilityKinds.length
    && mission.facilityKinds.every((kind) => context.facilityKinds.includes(kind))
    && context.conditionCodes.length === mission.conditions.length
    && mission.conditions.every((condition) => context.conditionCodes.includes(condition.code));
};
...
export const selectStageGate = (state: SessionState, stage: StageId): boolean => {
  switch (stage) {
    ...
    case 'resident-view': return state.evidence.selectedUnderservedZoneIds.length > 0 && hasAlternative(state);
    case 'opinion': return selectOpinionReady(state);
    default: return false;
  }
};
export const selectSessionSelectors = (state: SessionState) => ({ canAdvance: selectStageGate(state, STAGE_ORDER[Math.min(indexOfStage(state.stage) + 1, STAGE_ORDER.length - 1)]!), opinionReady: selectOpinionReady(state) as false });
export const selectCanAdvance = (state: SessionState): boolean => selectSessionSelectors(state).canAdvance;
...
const validPlacementAction = (state: SessionState, placement: FacilityPlacement): boolean => {
  const mission = missionForState(state); const city = state.cityId === null ? undefined : CITIES[state.cityId];
  return mission !== undefined && city !== undefined && placement.slotId.trim().length > 0 && mission.facilityKinds.includes(placement.facilityKind) && city.candidates.some((candidate) => candidate.id === placement.candidateId);
};
...
export function sessionReducer(state: SessionState, action: SessionAction): SessionState {
  switch (action.type) {
    ...
    case 'place-facility': {
      if (!validPlacementAction(state, action.placement)) return state;
    ...
    case 'store-analysis': return isFreshAnalysis(state, action.analysis) ? { ...state, analysis: copyAnalysis(action.analysis) } : state;
    ...
    case 'go-to-stage': {
      ...
      if (target < current) return { ...state, stage: action.stage }; if (target !== current + 1 || !selectStageGate(state, action.stage)) return state; return { ...state, stage: action.stage };
    }
