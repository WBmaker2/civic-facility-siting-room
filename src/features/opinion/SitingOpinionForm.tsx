  ...
  const textField = (key: 'rationale' | 'counterargument' | 'mitigation', label: string, frame: string) => {
    ...
    return (
      <div className="opinion-field">
        ...
        <textarea id={`opinion-${key}`} value={safeDraft[key]} maxLength={300} minLength={10} aria-invalid={error === null ? undefined : true} aria-describedby={error === null ? undefined : errorId(key)} onChange={(event) => update(key, event.target.value)} />
  ...
  return (
    <section aria-labelledby="opinion-form-heading" data-stage-id="opinion" role="region">
      ...
      <div className="opinion-field">
        ...
        <select id="opinion-zone" value={safeDraft.underservedZoneId ?? ''} aria-invalid={validation.errors.underservedZone === null ? undefined : true} aria-describedby={validation.errors.underservedZone === null ? undefined : errorId('underservedZone')} onChange={(event) => update('underservedZoneId', event.target.value || null)}>
