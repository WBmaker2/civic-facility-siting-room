...
import { chooseIntake, fillOpinion, installConsoleGuards, inspectAndOpenResident, placeSingle, reviewLayers } from './flow-helpers';
...
test('completes combined mission with shared budget and a phased second pair', async ({ page }) => {
  await page.getByRole('combobox', { name: '미션 선택' }).selectOption('combined-review');
  await page.getByRole('radio', { name: '안전' }).check();
  await expect(page.getByText(/공유 예산.*4토큰/)).toBeVisible();
  ...
  await page.getByRole('button', { name: '건강 도움소 1곳 시설 배치' }).click();
  await expect(page.getByText('남은 예산 토큰 2')).toBeVisible();
  await page.getByRole('button', { name: '영향 계산' }).click();
  await inspectAndOpenResident(page);
  await page.getByRole('radio', { name: /새길 동쪽 구역/ }).check();
  await page.getByRole('button', { name: 'B안 저장' }).click();
  await expect(page.getByRole('heading', { name: 'A안과 B안 비교' })).toBeVisible();
  await expect(page.locator('.proposal-column')).toHaveCount(2);
  await expect(page.getByText(/예산 토큰 4개 안에 놓기: 충족/)).toHaveCount(2);
  await expect(page.getByText(/도달 불가 구역 없이 놓기: 충족/)).toHaveCount(2);
  await fillOpinion(page, 'B안', '한 시설을 먼저 설치하고 다른 시설은 단계적으로 설치합니다.');
  await expect(page.getByLabel('완성한 입지 심의 의견서').getByText('한 시설을 먼저 설치하고 다른 시설은 단계적으로 설치합니다.', { exact: true })).toBeVisible();
  await expect(page.getByText('타당안—절충 확인')).toBeVisible();
});
