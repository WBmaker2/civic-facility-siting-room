...
- `npm run test:e2e`: PASS — 5 tests, 8.0s. 모든 E2E에는 page error 및 console error/warning fail-closed guard가 있습니다 (`tests/e2e/flow-helpers.ts`).
  ...
  - `table-only.spec.ts`: grid를 활성화하지 않고 labeled table에서 후보를 선택해 분석·소외 구역·대안·의견을 완료했습니다. population/road/risk/cost/existing/candidate/analysis/resident/opinion 텍스트를 확인합니다.
  - `mobile-and-motion.spec.ts`: 375×812에서 document overflow 0, 지도/표/선택 위치/결과표 탭, 선택 좌표 B2, visible focus ring, root text-scale reflow, reduced-motion computed animation `none`/`0s`를 확인했습니다.
...
- `npm run lint`: PASS.
- `npm run test:unit`: PASS — 25 files, 224 tests (E2E specs are excluded from Vitest discovery).
- `npm run check:lines`: PASS — 91 source files, 0 files at or above 500 lines.
...
5. 375px, keyboard/tab focus, 200% reflow proxy, reduced motion 자동 검증 PASS. VoiceOver와 물리적 macOS Reduce Motion은 아래와 같이 미검증입니다.
6. 가상 도시 2종, 미션 4개, 인구·도로·위험·비용·기존 시설 5개 데이터층, 기존 시설 문맥, combined shared budget — `src/domain/fixtures.test.ts`, `learner-flow.spec.ts`.
...
UNVERIFIED — 사용자 실기 확인 필요. 이 세션에서는 macOS VoiceOver 및 물리적 Reduce Motion을 수행하지 않았으며 PASS로 주장하지 않습니다. 자동 axe, accessibility semantics, keyboard interaction, Playwright viewport/text-scale 및 `prefers-reduced-motion` 근거와 구분합니다.
...
# 브라우저 확대 200%와 시스템 Reduce Motion을 각각 켜고 focus/overflow/정적 범위를 확인
```

확인 전까지 VoiceOver 버전과 물리적 관찰 결과는 비워 둡니다.

## 변경 범위

Task 14 이후 acceptance failure로 확인된 375px 컨테이너 가로 넘침을 `src/styles/responsive.css`에서 내부 overflow/reflow로 최소 수정했습니다. 이에 맞춰 2026-08-27 `개선` update-history 항목을 추가했습니다. 테스트·검증 파일은 외부 네트워크, persistence, analytics, map SDK, API, login, submission을 추가하지 않습니다.
